...
def parse_args():
    ...
    parser.add_argument('--npu_profiling', type=bool, default=False, help='profiling for performance or not')
...
def main():
    ...
    if args.npu_profiling:
        work_dir = os.getcwd()
        profiling_dir = os.path.join(work_dir, "npu_profiling")
        ## Check profiling output directory 
        if not os.path.isdir(profiling_dir):
            os.makedirs(profiling_dir)
        
        ## For different Net, the fp and bp node maybe different, please change them.
        fp_node_name = "discriminator/init_resblock/res1/conv_0/Conv2D"
        bp_node_name = "gradients/discriminator/init_resblock/res1/conv_0/Conv2D_grad/Conv2DBackpropFilter"
        options = '{"output": "%s", \
                    "training_trace": "on", \
                    "task_trace": "on", \
                    "aicpu": "on", \
